...
in vec4 v_projectedTexcoord;   // Coordenadas projetadas interpoladas do vertex shader (usadas para o cálculo de sombras)
in vec3 v_normal;              // Vetor normal interpolado do vertex shader (usado para cálculo de iluminação)
in vec3 v_surfaceToView;       // Vetor da superfície até a visão interpolado do vertex shader (não usado diretamente aqui)

uniform vec4 u_colorMult;      // Multiplicador de cor aplicado à textura base (usado para ajustar a cor final)
uniform sampler2D u_texture;   // Textura base que define a cor do fragmento
uniform sampler2D u_projectedTexture; // Textura de profundidade do mapa de sombras projetada
uniform float u_bias;          // Bias aplicado à profundidade para evitar artefatos de sombras (shadow acne)
uniform float u_penumbralSize; // Tamanho da penumbra (transição suave de sombras)
uniform vec3 u_reverseLightDirection; // Direção da luz reversa (para iluminação direcional)

out vec4 outColor;             // Cor final calculada para o fragmento (pixel)

const float PENUMBRA_SIZE = 0.06; // Valor fixo para o tamanho da penumbra (ajuste conforme necessário)

void main() {
    // Normaliza o vetor normal interpolado para garantir precisão no cálculo da iluminação
    vec3 normal = normalize(v_normal);

    // Calcula a iluminação direcional usando o produto escalar entre a direção da luz e o vetor normal
    // O valor resultante é clamped para [0,1] usando a função max
    float light = max(dot(normal, u_reverseLightDirection), 0.0);

    // Converte as coordenadas projetadas de 4D (vec4) para 3D (vec3) dividindo pelo componente W
    vec3 projectedTexcoord = v_projectedTexcoord.xyz / v_projectedTexcoord.w;
    
    // Adiciona o bias à profundidade atual para evitar artefatos de sombras
    float currentDepth = projectedTexcoord.z + u_bias;

    // Verifica se as coordenadas X e Y projetadas estão dentro do intervalo [0, 1] da textura de sombras
    bool inRange = projectedTexcoord.x >= 0.0 && projectedTexcoord.x <= 1.0 &&
                   projectedTexcoord.y >= 0.0 && projectedTexcoord.y <= 1.0;

    // Obtém a profundidade correspondente do mapa de sombras na posição projetada
    float projectedDepth = texture(u_projectedTexture, projectedTexcoord.xy).r;

    // Calcula a diferença entre a profundidade projetada (do mapa de sombras) e a profundidade atual
    float depthDifference = projectedDepth - currentDepth;

    // Usa a função smoothstep para calcular a penumbra, suavizando a transição de sombras
    float penumbra = smoothstep(0.0, PENUMBRA_SIZE, depthDifference);
    // Determina se o ponto está na sombra total (umbra) ou na penumbra, ou define um valor padrão de sombra
    float shadow = (inRange) ? (depthDifference < 0.0 ? 0.0 : penumbra) : 0.6;

    // Obtém a cor da textura base na coordenada de textura interpolada e aplica o multiplicador de cor
    vec4 texColor = texture(u_texture, v_texcoord) * u_colorMult;

    // Multiplica a cor da textura pela iluminação calculada e pela sombra, e define a cor final do fragmento
