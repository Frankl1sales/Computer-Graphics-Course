...
out vec2 v_shadowCoord;  // Coordenadas para sombras
...
void main() {
    ...
    vec3 lightPos = vec3(0.0, 0.0, 1.0); // Posição da luz, ajuste conforme necessário
    vec3 lightDir = normalize(lightPos - worldPosition.xyz);
    vec3 shadowCoord = worldPosition.xyz + lightDir * 0.1; // Ajusta o deslocamento conforme necessário
    v_shadowCoord = shadowCoord.xy / shadowCoord.z; // Normaliza as coordenadas
}
`;
...
void main() {
    ...
    // Obtém a profundidade projetada e aplica a suavização das sombras
    float projectedDepth = texture(u_projectedTexture, projectedTexcoord.xy).r;

    // Calcula a suavização da sombra
    float shadow = smoothstep(0.0, 1.0, projectedDepth - currentDepth);
